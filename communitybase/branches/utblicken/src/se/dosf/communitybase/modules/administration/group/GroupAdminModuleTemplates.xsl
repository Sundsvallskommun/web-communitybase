				...
				<div class="noBorderDiv" style="padding: 2px 2px 2px 3px;">
					...
					<div class="floatleft">
						<select name="GroupAccessLevel" onchange="this.options[this.selectedIndex].value != 'MEMBER' ? document.getElementById('commenttext').innerHTML = '{$user.update.comment.admin}:' : document.getElementById('commenttext').innerHTML = '{$user.update.comment.member}:'">
							...
							<xsl:if test="../Roles[Role = 'USER']">
								<option value="USER">					
									<xsl:choose>
										<xsl:when test="../requestparameters">
											<xsl:if test="../requestparameters/parameter[name='GroupAccessLevel']/value = 'USER'">
												<xsl:attribute name="selected"/>
											</xsl:if>						
										</xsl:when>
										<xsl:otherwise>
											<xsl:if test="communityUserAttributes/groups/group[groupID = /document/group/groupID]/GroupRelation/GroupAccessLevel = 'USER'">
												<xsl:attribute name="selected"/>
											</xsl:if>
										</xsl:otherwise>
									</xsl:choose>
									<xsl:value-of select="$user.update.user" />
								</option>
							</xsl:if>
							<xsl:if test="../Roles[Role = 'PUBLISHER']">
							...
							<xsl:if test="/document/isSchoolAdmin and ../Roles[Role = 'ADMIN']">
								<option value="ADMIN">				
									<xsl:choose>
										<xsl:when test="../requestparameters">
		...
		<div class="adminDiv">
			...
			<form method="POST" name="moveusers" action="{/document/requestinfo/contextpath}{/document/section/fullAlias}/{/document/module/alias}/{/document/group/groupID}/moveUsers">
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						<div class="noBorderDiv"><p><xsl:value-of select="$groupInfoElement.users.nousers" /></p></div>
					</xsl:otherwise>
				</xsl:choose>
			...
			<div class="noBorderDiv">
				<form method="post" name="adduser" action="{/document/requestinfo/contextpath}{/document/section/fullAlias}/{/document/module/alias}/{/document/group/groupID}/addUser">
					<table class="floatleft">
						...
						<tr>
							<td>
								<input type="text" name="email" size="40" value="{requestparameters/parameter[name='email']/value}"/>
							</td>
							<td>
								<select name="GroupAccessLevel" onchange="this.options[this.selectedIndex].value != 'MEMBER' ? document.getElementById('commenttext').innerHTML = '{$groupInfoElement.adduser.admin.comment}:' : document.getElementById('commenttext').innerHTML = '{$groupInfoElement.adduser.member.comment}:'">
									...
									<xsl:if test="Roles[Role = 'USER']">
										<option value="USER">
											...
											<xsl:value-of select="$groupInfoElement.adduser.user" />
										</option>
									</xsl:if>
									<xsl:if test="Roles[Role = 'PUBLISHER']">
										<option value="PUBLISHER">
							...
							<td>
								<input type="text" name="comment" size="30" value="{requestparameters/parameter[name='comment']/value}"/>
							</td>
							<td>
		...
		<div class="floatleft full marginbottom border">
			...
			<div class="floatright marginright">
				...
				<xsl:choose>
					...
					<xsl:when
						test="communityUserAttributes/groups/group[groupID = /document/group/groupID]/GroupRelation[GroupAccessLevel = 'USER']">
						<xsl:value-of select="$user.user" />
					</xsl:when>
					<xsl:when
		...
		<div class="floatleft full marginbottom border">
			...
			<div class="floatright marginright">
				...
				<xsl:choose>
					...
					<xsl:when
						test="groups/group[groupID = /document/group/groupID]/GroupRelation[GroupAccessLevel = 'USER']">
						<xsl:value-of select="$invitation.user" />
					</xsl:when>
					<xsl:when
