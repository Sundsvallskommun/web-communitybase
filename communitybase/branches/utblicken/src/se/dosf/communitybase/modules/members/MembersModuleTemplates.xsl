...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="Members">
		...
		<xsl:if test="user/communityUserAttributes/groups/group[groupID = /document/group/groupID]/GroupRelation[GroupAccessLevel = 'USER']">
			<h2><xsl:value-of select="$Members.users" /></h2>
			<xsl:apply-templates select="user[communityUserAttributes/groups/group[groupID = /document/group/groupID]/GroupRelation[GroupAccessLevel = 'USER']]"/>
		</xsl:if>
	...
	<xsl:template match="user">
		<div class="divNormal">
			<div class="normalTableHeading floatleft" style="width: 99%;">
				<b>
