...
public class GroupFirstpageDAO extends BaseDAO {
	...
	public List<IdentifiedEvent> getEvents(CommunityGroup group, Timestamp lastLogin) throws SQLException {
		...
		ArrayListQuery<IdentifiedEvent> query = new ArrayListQuery<IdentifiedEvent>(dataSource, true, "SELECT groupID as id, title as description, title, changed as added FROM groupfirstpage WHERE groupID = ? AND changed > ? ORDER by date", EVENT_POPULATOR);
