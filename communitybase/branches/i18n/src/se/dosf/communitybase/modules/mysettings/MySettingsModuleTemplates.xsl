...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="MySettingsModule">
		<form method="post" name="mysettings" id="mysettings"
			action="{/document/requestinfo/url}">

			<p class="info">
				<img
					src="{/document/requestinfo/contextpath}/static/f/{/document/module/sectionID}/{/document/module/moduleID}/pics/information.png" />
				<xsl:text>&#x20;</xsl:text>
				<xsl:value-of select="$MySettingsModule.information1" />
				.
				<br />
			...
			<p class="info">
				<img
					src="{/document/requestinfo/contextpath}/static/f/{/document/module/sectionID}/{/document/module/moduleID}/pics/warning.png" />
				<xsl:text>&#x20;</xsl:text>
	...
	<xsl:template match="user" mode="form">
		...
		<div class="noBorderDiv">
			...
			<div class="full bigmarginbottom">
				<b>
					<xsl:value-of select="$user.personinfo" />
				</b>
				<br />
				<hr />
			</div>
						...
						<xsl:attribute name="value">
								<xsl:choose>
									<xsl:when test="../requestparameters">
										<xsl:value-of
							select="../requestparameters/parameter[name='firstname']/value" />
									</xsl:when>
									<xsl:otherwise>
					...
					<input type="text" name="lastname" size="40">
						<xsl:attribute name="value">
								<xsl:choose>
									<xsl:when test="../requestparameters">
										<xsl:value-of
							select="../requestparameters/parameter[name='lastname']/value" />
									</xsl:when>
									<xsl:otherwise>
					...
					<input type="text" name="phoneHome" size="40">
						<xsl:attribute name="value">
								<xsl:choose>
									<xsl:when test="../requestparameters">
										<xsl:value-of
							select="../requestparameters/parameter[name='phoneHome']/value" />
									</xsl:when>
									<xsl:otherwise>
					...
					<input type="text" name="phoneMobile" size="40">
						<xsl:attribute name="value">
								<xsl:choose>
									<xsl:when test="../requestparameters">
										<xsl:value-of
							select="../requestparameters/parameter[name='phoneMobile']/value" />
									</xsl:when>
									<xsl:otherwise>
					...
					<input type="text" name="phoneWork" size="40">
						<xsl:attribute name="value">
								<xsl:choose>
									<xsl:when test="../requestparameters">
										<xsl:value-of
							select="../requestparameters/parameter[name='phoneWork']/value" />
									</xsl:when>
									<xsl:otherwise>
			...
			<div class="full bigmarginbottom">
				<b>
					<xsl:value-of select="$user.accountinfo" />
				</b>
				<br />
				<hr />
			</div>
					...
					<input type="text" name="email" size="40">
						<xsl:attribute name="value">
								<xsl:choose>
									<xsl:when test="../requestparameters">
										<xsl:value-of
							select="../requestparameters/parameter[name='email']/value" />
									</xsl:when>
									<xsl:otherwise>
			...
			<div class="floatleft bigmarginbottom" style="width: 20%">
				<input type="checkbox" name="changepassword"
					onclick="document.mysettings.password.disabled=!this.checked; document.mysettings.passwordReference.disabled=!this.checked">
					<xsl:if
						test="../requestparameters/parameter[name='changepassword'][value='on']">
			...
			<div class="bigmarginbottom">
				<div class="floatleft" style="width: 20%">
					<label for="password">
						<xsl:value-of select="$user.newpassword" />
						:
						<br />
						(
						<xsl:value-of select="$user.minimumchars" />
						)
					</label>
				</div>
				<div>
					<input type="password" name="password"
						value="{../requestparameters/parameter[name='password']/value}"
						size="40">
						<xsl:choose>
							<xsl:when test="../requestparameters">
								<xsl:if
									test="not(../requestparameters/parameter[name='changepassword'])">
			...
			<div class="bigmarginbottom">
				<div class="floatleft" style="width: 20%">
					<label for="password">
						<xsl:value-of select="$user.confirmpassword" />
						:
					</label>
				</div>
				<div>
					<input type="password" name="passwordReference"
						value="{../requestparameters/parameter[name='passwordReference']/value}"
						size="40">
						<xsl:choose>
							<xsl:when test="../requestparameters">
								<xsl:if
			...
			<div class="full bigmarginbottom">
				<b>
					<xsl:value-of select="$user.emailresume.header" />
				</b>
				<br />
				<hr />
			</div>
			<div class="bigmarginbottom">
				<div class="floatleft" style="width: 40%">
					<input type="checkbox" name="emailresume"
						onclick="document.mysettings.resumetime.disabled=!this.checked">
				...
				<div>
					...
					<xsl:choose>
						...
						<xsl:otherwise>
							<script language="javascript">
								document.mysettings.resumetime.selectedIndex =
								<xsl:value-of select="communityUserAttributes/resume" />
								;
							</script>
						</xsl:otherwise>
					</xsl:choose>
	...
	<xsl:template match="language">
		...
		<option value="{code}">
			<xsl:if test="$requestparameters/parameter[name='languageCode']/value = code or not($requestparameters) and $user/language/code = code">
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:text><xsl:value-of
							select="$validationError.unknownValidationErrorType" /></xsl:text>
					</xsl:otherwise>
				</xsl:choose>
			...
			<p class="error">
				<xsl:choose>
					<xsl:when test="messageKey='UpdateFailedUserNotFound'">
						<xsl:value-of
							select="$validationError.messageKey.UpdateFailedUserNotFound" />
						!
					</xsl:when>
