...
public class RegistrationModule extends AnnotatedForegroundModule {
	...
	private static final AnnotatedRequestPopulator<CommunityUser> POPULATOR = new AnnotatedRequestPopulator<CommunityUser>(CommunityUser.class, new LanguagePopulator());
	...
	public ForegroundModuleResponse defaultMethod(HttpServletRequest req, HttpServletResponse res, User user, URIParser uriParser) throws Exception {
		...
		if (uriParser.size() != 3) {
			throw new URINotFoundException(this.sectionInterface, moduleDescriptor, uriParser);
		}
		...
		if (req.getMethod().equalsIgnoreCase("POST")) {
			try {
				...
				List<GlobalAdminModule> globalAdminModules = this.getGlobalAdminModules();
				
				if(globalAdminModules != null){
					
					if((invitation.getSchools() != null && !invitation.getSchools().isEmpty()) || invitation.isAdmin()){
						
						for(GlobalAdminModule globalAdminModule : globalAdminModules){
	...
	public List<GlobalAdminModule> getGlobalAdminModules() {

		List<GlobalAdminModule> globalAdminModules = ModuleUtils.getCachedModules(sectionInterface.getSystemInterface().getRootSection(), GlobalAdminModule.class, true);

		if (globalAdminModules == null) {

			log.warn("Module " + moduleDescriptor + " unable to find any instance of GlobalAdminModule!");
		}
