...
//	public void addGroupInvitation(String email, CommunityGroup group, GroupRelation groupRelation, Timestamp expires) throws SQLException {
//		this.addGroupInvitation(email, null, group, groupRelation, expires);
//	}
	
	public void addGroupInvitation(String email, Language language, CommunityGroup group, GroupRelation groupRelation, Timestamp expires, boolean useGroupEmail) throws SQLException {
	...
	private void add(Invitation invitation,	TransactionHandler transactionHandler) throws SQLException {
		...
		query.setString(2, invitation.getLanguage() == null ? null : invitation.getLanguage().getLanguageCode());
		query.setTimestamp(3, invitation.getExpires());
		query.setTimestamp(4, invitation.getUpdated());
		query.setTimestamp(5, invitation.getLastSent());
		query.setString(6, invitation.getLinkID());
		query.setBoolean(7, invitation.isAdmin());
		query.setBoolean(8, invitation.isResend());
		if(invitation.getSenderEmail() != null){
			query.setString(9, invitation.getSenderEmail());
		}else{
			query.setNull(9, 0);
		}
	...
	public void update(Invitation invitation, boolean schools, boolean groups, TransactionHandler transactionHandler) throws SQLException {
		...
		UpdateQuery query = transactionHandler.getUpdateQuery("UPDATE invitations SET email = ?, languageCode = ?, expires = ?, updated = ?, lastSent = ?, linkID = ?, admin = ?, resend = ?, senderEmail = ? WHERE invitationID = ?");
		...
		query.setInt(10,invitation.getInvitationID());
...
//		this.addAdminInvitation(email, null, expires);
//	}

	public void addAdminInvitation(String email, Language language, Timestamp expires) throws SQLException {
...
//		this.addSchoolInvitation(email, null, school, expires);
//	}
	
	public void addSchoolInvitation(String email, Language language, School school, Timestamp expires) throws SQLException {
