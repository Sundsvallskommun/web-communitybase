...
public class InvitationModule extends AnnotatedForegroundModule {
	...
	@ModuleSetting(allowsNull = true)
	@TextFieldSettingDescriptor(name = "Registration approval title", description = "Registration approval title", required = false)
	private String registrationApprovalTitle;
	...
	@ModuleSetting(allowsNull = true)
	@TextFieldSettingDescriptor(name = "Modal link label", description = "Modal link label text", required = false)
		...
		if (invitationID != null && uriParser.get(3) != null && StringUtils.isValidUUID(uriParser.get(3))) {
			...
			if (invitation != null) {
				...
				if (useRegistrationApproval) {
					...
					XMLUtils.appendNewElement(doc, registerElement, "RegistrationApprovalTitle", registrationApprovalTitle);
					XMLUtils.appendNewElement(doc, registerElement, "RegistrationApprovalText", registrationApprovalText);
					XMLUtils.appendNewElement(doc, registerElement, "ModalLinkLabel", modalLinkLabel);
				}
	...
	public boolean sendInvitation(Invitation invitation, HttpServletRequest req) throws SQLException {
		...
		try {
			...
			mapTagSource.addTag(INVITATION_LINK_TAG, emailResumeModule.getFullContextPath() + this.getFullAlias() + "/register/" + invitation.getInvitationID() + "/" + invitation.getLinkID());
