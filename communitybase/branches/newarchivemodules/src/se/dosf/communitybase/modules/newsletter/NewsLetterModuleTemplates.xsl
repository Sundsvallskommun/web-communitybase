...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
					...
					<span>
						<xsl:choose>
							<xsl:when test="NewsLetter">
								...
								<xsl:if test="NewsLetter/draft = 'true'">
									<xsl:text>&#160;</xsl:text>
									<xsl:value-of select="$newsletter.draft.title"/>
								</xsl:if>								
	...
	<xsl:template match="NewsLetter" mode="show">
		...
		<xsl:if test="/Document/hasUpdateAccess or /Document/hasReceiptAccess">
			<div class="floatright">
				<xsl:if test="/Document/hasReceiptAccess">
		...
		<div class="margintop">
			<xsl:if test="imageLocation = 'ABOVE'">
				<p class="text-align-center">
					<img src="{$modulePath}/getImage/{newsletterID}" />
				</p>
			</xsl:if>
	...
	<xsl:template match="AddNewsLetter">
		...
		<form method="post" name="addNewsletter" id="addNewsletter" action="{/Document/requestinfo/uri}" enctype="multipart/form-data" accept-charset="ISO-8859-1">
			...
			<div class="content-box">
				<h1 class="header">
					<span><xsl:value-of select="$addNewsletter.header" /></span>
				</h1>
				...
				<div class="content">
					...
					<p>
						<xsl:value-of select="$newsletter.publishing" />:<br />
						
						<xsl:call-template name="createRadio">
							<xsl:with-param name="name" select="'draft'"/>
							<xsl:with-param name="id" select="'publish'"/>
							<xsl:with-param name="value" select="'false'"/>
							<xsl:with-param name="checked" select="'true'"/>
						</xsl:call-template>
						
						<label for="publish"><xsl:value-of select="$newsletter.published1" /></label><br />
						...
						<label for="draft"><xsl:value-of select="$newsletter.saveAsDraft" /></label>
					</p>					
	...
	<xsl:template match="UpdateNewsLetter">
		...
		<form method="post" name="updateNewsletter" id="updateNewsletter" action="{/Document/requestinfo/uri}" enctype="multipart/form-data" accept-charset="ISO-8859-1">
			...
			<div class="content-box">
				<h1 class="header">
					<span><xsl:value-of select="$updateNewsletter.header" />:<xsl:text>&#x20;</xsl:text><xsl:value-of select="NewsLetter/title" /></span>
				</h1>
	...
	<xsl:template match="ShowReadReceipt">
		...
		<div class="content-box">
			<h1 class="header">
				<span><xsl:value-of select="$showReadReceipt.header" />:<xsl:text>&#x20;</xsl:text><xsl:value-of select="NewsLetter/title" /></span>
			</h1>
			
			<div class="content">
			
				<p><xsl:value-of select="$showReadReceipt.summary.part1" /><xsl:text>&#x20;</xsl:text><b><xsl:value-of select="count(NewsLetter/receipts/receipt) + HiddenReceipts" /></b><xsl:text>&#x20;</xsl:text><xsl:value-of select="$showReadReceipt.summary.part2" /><xsl:text>&#x20;</xsl:text><b><xsl:value-of select="HiddenReceipts" /></b><xsl:text>&#x20;</xsl:text><xsl:value-of select="$showReadReceipt.summary.part3" />.</p>
				...
				<xsl:choose>
					...
					<xsl:when test="HiddenReceipts > 0">
						<xsl:value-of select="$showReadReceipt.noreceiptAccess" />
					</xsl:when>					
					<xsl:otherwise>
