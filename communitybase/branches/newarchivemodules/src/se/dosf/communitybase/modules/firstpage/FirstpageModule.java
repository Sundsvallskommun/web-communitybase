...
public class FirstpageModule extends AnnotatedForegroundModule implements ForegroundModuleCacheListener, SystemStartupListener {
	...
	public static final String VERSION_PREFIX = "CommunityBase 2.0.13";
	...
	private void scanModules(SectionInterface sectionInterface) throws Exception {
		...
		for (Entry<ForegroundModuleDescriptor, ForegroundModule> entry : this.sectionInterface.getForegroundModuleCache().getCachedModules()) {
			...
			try {
				...
				if(moduleEntry.getValue().getModuleType() != ModuleType.Hidden){
					...
					if (fullGroupAlias != null) {
						...
						if (moduleType == ModuleType.Public && AnnotatedCommunityModule.class.isAssignableFrom(moduleEntry.getValue().getClass()) ? ((AnnotatedCommunityModule) moduleEntry.getValue()).moduleEnabled(group) : communityModuleDAO.isModuleEnabled(moduleEntry.getKey().getModuleID(), group)) {
						...
						} else if((moduleType == ModuleType.Public && AccessUtils.checkAccess(moduleEntry.getKey(), group)) || moduleType == ModuleType.Administration) {
