...
import se.dosf.communitybase.modules.oldcontentremover.OldContentRemovalModule;
...
public class GroupAdminModule extends AnnotatedAdminModule<CommunityGroup> implements CommunityModule, InstanceListener<OldContentRemovalModule> {
	...
	protected OldContentRemovalModule oldContentRemovalModule;
	...
	public void init(ForegroundModuleDescriptor moduleDescriptorBean, SectionInterface sectionInterface, DataSource dataSource) throws Exception {
		...
		systemInterface.getInstanceHandler().addInstanceListener(OldContentRemovalModule.class, this);
	}
	...
	public void unload() throws Exception {
		...
		systemInterface.getInstanceHandler().removeInstanceListener(OldContentRemovalModule.class, this);
	}
	...
	public ForegroundModuleResponse defaultMethod(HttpServletRequest req, HttpServletResponse res, CommunityUser user, URIParser uriParser, CommunityGroup group, ValidationException validationException) throws SQLException {
		...
		if (firstpageModule != null) {
			...
			if (!moduleMap.isEmpty()) {
				...
				if (oldContentRemovalModule != null) {
					Element oldContentElement = doc.createElement("OldContentRemover");
					groupInfoElement.appendChild(oldContentElement);

					XMLUtils.appendNewElement(doc, oldContentElement, "URL", oldContentRemovalModule.getFullAlias());
					XMLUtils.appendNewElement(doc, oldContentElement, "Count", oldContentRemovalModule.getOldContentCount(user, group, DateUtils.getTodaysDatePlusMinusDays(-365)));
				}
			}
		}
		...
		if (req.getMethod().equalsIgnoreCase("POST")) {
			...
			try {
				...
				if (groupAccessLevel == GroupAccessLevel.ADMIN && !AccessUtils.checkAccess(user, group.getSchool())) {
					throw new AccessDeniedException("Group administrators are not allowed to add new administrators!");
				}
		...
		if (selectedUser != null) {
			...
			if (!AccessUtils.checkAccess(user, group.getSchool())) {
				...
				if (user.equals(selectedUser)) {
					...
					throw new AccessDeniedException("Group administrators are not allowed to edit other group administrators!");
				}
			}
			...
			if (req.getMethod().equalsIgnoreCase("POST")) {
				try {
					...
					if (groupAccessLevel == GroupAccessLevel.ADMIN && !AccessUtils.checkAccess(user, group.getSchool())) {
						throw new AccessDeniedException("Group administrators are not allowed to add new group administrators!");
					}
		...
		if (selectedUser != null) {
			...
			if (!AccessUtils.checkAccess(user, group.getSchool())) {
				...
				if (user.equals(selectedUser)) {
					...
					throw new AccessDeniedException("Group administrators are not allowed to delete other group administrators!");
				}
			}
	...
	public ForegroundModuleResponse moveUsers(HttpServletRequest req, HttpServletResponse res, CommunityUser user, URIParser uriParser, CommunityGroup group) throws SQLException, AccessDeniedException, IOException, URINotFoundException {
		return moveOrCopy(req, res, user, uriParser, group, true);
	}
	
	@GenericAdminMethod
	public ForegroundModuleResponse copyUsers(HttpServletRequest req, HttpServletResponse res, CommunityUser user, URIParser uriParser, CommunityGroup group) throws SQLException, AccessDeniedException, IOException, URINotFoundException {
		return moveOrCopy(req, res, user, uriParser, group, false);
	}
		
	private ForegroundModuleResponse moveOrCopy(HttpServletRequest req, HttpServletResponse res, CommunityUser user, URIParser uriParser, CommunityGroup group, boolean move) throws SQLException, AccessDeniedException, IOException, URINotFoundException {
		...
		if(req.getMethod().equalsIgnoreCase("POST")) {
			...
			if(uriParser.size() >= 3) {
				...
				if(uriParser.size() == 3) {
					...
					if(move){
						moveUsers = doc.createElement("MoveUsers");
					} else {
						moveUsers = doc.createElement("CopyUsers");
					}
					...
					if(move) {
						...
						return new SimpleForegroundModuleResponse(doc, moduleDescriptor.getName(), this.getBreadcrumb("Administrera " + group.getName(), "", "", group), this.getBreadcrumb("Kopiera användare", "", "copyUsers", group));
					}
						...
						for(CommunityUser selectedUser : users) {
							...
							if(selectedGroup.getUsers() == null || (!selectedGroup.getUsers().contains(selectedUser))) {
								...
								if(move){
									...
									log.info("User " + user  + " copying user " + selectedUser + " from group " + group + " to group " + selectedGroup);
								}
	...
	@Override
	public <InstanceType extends OldContentRemovalModule> void instanceAdded(Class<OldContentRemovalModule> key, InstanceType instance) {

		oldContentRemovalModule = instance;
	}

	@Override
	public <InstanceType extends OldContentRemovalModule> void instanceRemoved(Class<OldContentRemovalModule> key, InstanceType instance) {

		oldContentRemovalModule = null;
