...
public class GroupFirstpageDAO extends BaseDAO {
	...
	public void update(GroupFirstpage firstpage) throws SQLException{
		...
		UpdateQuery query = new UpdateQuery(this.dataSource, "INSERT INTO groupfirstpage VALUES(?,?,?,?,?,?) ON DUPLICATE KEY UPDATE title = ?, content = ?, changed = ?, image = ?, imagelocation = ?");
	...
	public void delete(GroupFirstpage firstpage) throws SQLException{
		...
		UpdateQuery query = new UpdateQuery(this.dataSource, "DELETE FROM groupfirstpage WHERE groupID = ?");
	...
	public GroupFirstpage get(CommunityGroup group) throws SQLException{
		...
		ObjectQuery<GroupFirstpage> query = new ObjectQuery<GroupFirstpage>(this.dataSource, "SELECT * FROM groupfirstpage WHERE groupID = ?", POPULATOR);
	...
	public List<IdentifiedEvent> getEvents(CommunityGroup group, Timestamp lastLogin) throws SQLException {
		...
		ArrayListQuery<IdentifiedEvent> query = new ArrayListQuery<IdentifiedEvent>(dataSource, "SELECT groupID as id, title as description, title, changed as added FROM groupfirstpage WHERE groupID = ? AND changed > ? ORDER by changed", EVENT_POPULATOR);
