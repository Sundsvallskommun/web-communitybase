...
public class CommunityModuleDAO extends BaseDAO {
	...
	public ArrayList<Integer> getEnabledModules(CommunityGroup group) throws SQLException {
		...
		if (group.isEnabled()) {
			...
			ArrayListQuery<Integer> query = new ArrayListQuery<Integer>(this.dataSource, sql, IntegerPopulator.getPopulator());
	...
	public List<Integer> getDistinctEnabledModules(List<CommunityGroup> groups) throws SQLException, IllegalArgumentException, IllegalAccessException {

		String sql = "SELECT DISTINCT moduleID FROM groupmodules WHERE groupID IN (" + StringUtils.toCommaSeparatedString(groups, CommunityGroup.GROUPID_FIELD) + ")";
	...
	public boolean isModuleEnabled(int moduleID, CommunityGroup group) throws SQLException {
		...
		if (group.isEnabled()) {
			...
			BooleanQuery query = new BooleanQuery(this.dataSource, "SELECT moduleID FROM groupmodules WHERE groupID = ? AND moduleID = ?");
			query.setInt(1, group.getGroupID());
	...
	public void disableModule(ModuleDescriptor moduleDescriptor, CommunityGroup group) throws SQLException {
		...
		UpdateQuery query = new UpdateQuery(dataSource, "DELETE FROM groupmodules WHERE groupID = ? AND moduleID = ?");
