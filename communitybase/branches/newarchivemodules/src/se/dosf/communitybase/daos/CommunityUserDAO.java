...
public class CommunityUserDAO extends BaseDAO {
	...
	public ArrayList<CommunityUser> getAll(boolean getSchools, boolean getGroups) throws SQLException {
		...
		ArrayListQuery<CommunityUser> query = new ArrayListQuery<CommunityUser>(this.dataSource, "SELECT * FROM users", POPULATOR);
	...
	public void delete(CommunityUser user) throws SQLException {
		...
		UpdateQuery query = new UpdateQuery(this.dataSource, "DELETE FROM users WHERE userID = ?");
	...
	public void update(CommunityUser user, boolean encryptPassword, boolean updateGroups, boolean updateSchools) throws SQLException {
		...
		try {
			...
			UpdateQuery query = transactionHandler.getUpdateQuery("UPDATE users SET email = ?, firstName = ?, lastName = ?, phoneHome = ?, phoneMobile = ?, phoneWork = ?, lastLogin = ?, password = " + (encryptPassword ? "PASSWORD(?)" : "?") + ", resume = ?, lastResume = ?,  admin = ?, enabled = ?, added = ? WHERE userID = ?");
			...
			query.setBoolean(12, user.isEnabled());
			query.setTimestamp(13, user.getAdded());
	...
	public ArrayList<CommunityUser> getUsers(CommunityGroup group, boolean getSchools, boolean getGroups) throws SQLException {
		...
		ArrayListQuery<CommunityUser> query = new ArrayListQuery<CommunityUser>(this.dataSource, "SELECT users.* FROM groupusers INNER JOIN users ON (groupusers.userID = users.userID) " + "WHERE (groupusers.groupID = ?) ORDER BY users.firstname, users.lastname;", POPULATOR);
	...
	public List<CommunityUser> getUsers(Collection<Integer> userIDList, boolean groups) throws SQLException {

		ArrayList<CommunityUser> users = new ArrayListQuery<CommunityUser>(this.dataSource, "SELECT * FROM users WHERE userID IN (" + StringUtils.toCommaSeparatedString(userIDList) + ") ORDER BY firstname, lastname", POPULATOR).executeQuery();

		if (users != null && (groups)) {
	...
	public List<CommunityUser> getUsers(List<Integer> userIDs, boolean getSchools, boolean getGroups, Connection connection) throws SQLException {

		String sql = "SELECT * FROM users WHERE userID IN(" + StringUtils.toQuotedCommaSeparatedString(userIDs) + ");";

		ArrayList<CommunityUser> users = new ArrayListQuery<CommunityUser>(connection, false, sql, POPULATOR).executeQuery();

		if (users != null) {

			if (getGroups) {
				for (CommunityUser user : users) {
					user.setGroups(this.groupDAO.getGroupRelations(user, connection));
				}
			}

			if (getSchools) {
				for (CommunityUser user : users) {
					user.setSchools(this.schoolDAO.getSchools(user, false, false, connection));
				}
			}
		}
	...
	public List<CommunityUser> getUsers(Integer groupID) throws SQLException {

		ArrayListQuery<CommunityUser> query = new ArrayListQuery<CommunityUser>(this.dataSource, "SELECT users.userID, users.`password`, users.firstname, users.lastname, users.email, users.admin, users.enabled, users.added, users.lastlogin, users.phoneHome, users.phoneWork, " + "users.resume, users.lastresume, users.phoneMobile " + "FROM groupusers INNER JOIN users ON (groupusers.userID=users.userID) " + "WHERE (groupusers.groupID = ?) ORDER BY users.firstname, users.lastname;", POPULATOR);
	...
	public CommunityUser findByUserID(Integer userID, boolean getSchools, boolean getGroups) throws SQLException {
		...
		ObjectQuery<CommunityUser> query = new ObjectQuery<CommunityUser>(this.dataSource, sql, POPULATOR);
	...
	public CommunityUser findByEmailPassword(String email, String password, boolean getSchools, boolean getGroups) throws SQLException {
		...
		ObjectQuery<CommunityUser> query = new ObjectQuery<CommunityUser>(this.dataSource, "SELECT * FROM users WHERE email = ? and password = PASSWORD(?)", POPULATOR);
	...
	public ArrayList<Integer> getUserIdsByAccessLevel(CommunityGroup group, GroupAccessLevel... accessLevel) throws SQLException {
		...
		ArrayListQuery<Integer> query = new ArrayListQuery<Integer>(this.dataSource, queryString, IntegerPopulator.getPopulator());
	...
	public ArrayList<Integer> getUserIdsFromGroupsByAccessLevel(List<Integer> groupIDs, GroupAccessLevel... accessLevel) throws SQLException {

		String queryString = "SELECT DISTINCT userID FROM groupusers WHERE groupID IN(" + StringUtils.toQuotedCommaSeparatedString(groupIDs) + ") AND accessLevel IN(" + StringUtils.toQuotedCommaSeparatedString(accessLevel) + ")";
	...
	public ArrayList<Integer> getAdminUserIdsFromSchools(List<Integer> schoolIDs) throws SQLException {

		String queryString = "SELECT DISTINCT userID FROM schooladmins WHERE schoolID IN(" + StringUtils.toQuotedCommaSeparatedString(schoolIDs) + ")";
	...
	public CommunityUser getUser(String email, boolean getSchools, boolean getGroups) throws SQLException {
		...
		ObjectQuery<CommunityUser> query = new ObjectQuery<CommunityUser>(this.dataSource, "SELECT * FROM users WHERE email = ?", POPULATOR);
	...
	public List<CommunityUser> getAdminUsers(boolean getSchools, boolean getGroups) throws SQLException {
		...
		ArrayListQuery<CommunityUser> query = new ArrayListQuery<CommunityUser>(this.dataSource, "SELECT * FROM users WHERE admin = true", POPULATOR);
	...
	public ArrayList<CommunityUser> getUsers(School school) throws SQLException {
		...
		ArrayListQuery<CommunityUser> query = new ArrayListQuery<CommunityUser>(this.dataSource, "SELECT users.* FROM schooladmins INNER JOIN users ON (schooladmins.userID=users.userID) WHERE schooladmins.schoolID = ?", POPULATOR);
	...
	public ArrayList<CommunityUser> getUsers(School school, Connection connection) throws SQLException {

		ArrayListQuery<CommunityUser> query = new ArrayListQuery<CommunityUser>(connection, false, "SELECT users.* FROM schooladmins INNER JOIN users ON (schooladmins.userID=users.userID) WHERE schooladmins.schoolID = ?", POPULATOR);
	...
	public Collection<Integer> getAdminAndSchoolAdminIDs() throws SQLException {
		...
		return new ArrayListQuery<Integer>(dataSource, "SELECT users.userID FROM users LEFT OUTER JOIN schooladmins ON (users.userID=schooladmins.userID) WHERE (users.admin = true) OR (schooladmins.schoolID IS NOT NULL)", IntegerPopulator.getPopulator()).executeQuery();
	}

	public List<CommunityUser> getUsers(boolean getSchools, boolean getGroups, boolean getSchoolAdmins, List<GroupAccessLevel> accessLevels) throws SQLException {
		
		String sql =  "SELECT DISTINCT users.* FROM users " +
				"LEFT OUTER JOIN schooladmins ON (users.userID=schooladmins.userID) " +
				"WHERE users.userID IN(SELECT userID FROM groupusers WHERE accessLevel IN(" + StringUtils.toQuotedCommaSeparatedString(accessLevels) + ")) ";
		
		if(getSchoolAdmins) {
			sql += "OR (schooladmins.schoolID IS NOT NULL)";
		}
		
		List<CommunityUser> users = new ArrayListQuery<CommunityUser>(dataSource, sql, POPULATOR).executeQuery();
		
		if (!CollectionUtils.isEmpty(users) && (getSchools || getGroups)) {
			...
			for(CommunityUser user : users){
				...
				if(getSchools){
					user.setSchools(this.schoolDAO.getSchools(user, false, true));
				}
			}
		}
	...
	public ArrayList<CommunityUser> getUsers(int resumeHour) throws SQLException {
		...
		ArrayListQuery<CommunityUser> query = new ArrayListQuery<CommunityUser>(this.dataSource, "SELECT * FROM users WHERE resume = ?", POPULATOR);
	...
	public void setLastResume(Integer userID, Timestamp lastResume) throws SQLException {
		...
		UpdateQuery query = new UpdateQuery(dataSource, "UPDATE users SET lastResume = ? WHERE userID = ?");
	...
	public int getUserCount() throws SQLException {
		...
		ObjectQuery<Integer> query = new ObjectQuery<Integer>(this.dataSource, "SELECT COUNT(userID) FROM users", IntegerPopulator.getPopulator());
	...
	public List<Character> getUserFirstLetterIndex(UserField filteringField) throws SQLException {
		...
		return new ArrayListQuery<Character>(this.dataSource, "SELECT DISTINCT UPPER(LEFT(" + filteringField.toString().toLowerCase() + ", 1)) as letter FROM users ORDER BY letter ", CharacterPopulator.getPopulator()).executeQuery();
	}
	
	public List<CommunityUser> getUsers(UserField filteringField, Order order, char startsWith, boolean groups) throws SQLException {
		
		String field = filteringField.toString().toLowerCase();
		
		String sql = "SELECT * FROM users WHERE " + field + " LIKE '" + startsWith + "%' ORDER BY " + field + " " + order;
		
		ArrayListQuery<CommunityUser> query = new ArrayListQuery<CommunityUser>(this.dataSource, sql, POPULATOR);

		List<CommunityUser> users = query.executeQuery();
	...
	public List<? extends User> searchUsers(String search, boolean groups, boolean attributes, Integer maxHits, List<Integer> groupIDs, boolean onlyEnabledUsers) throws SQLException {

		// TODO add support for search by groupIDs?
		
		StringBuilder sql = new StringBuilder("SELECT * FROM users WHERE firstName LIKE ? OR lastName LIKE ?");
		...
		ArrayListQuery<CommunityUser> query = new ArrayListQuery<CommunityUser>(this.dataSource, sql.toString(), POPULATOR);
