				...
				<xsl:if test="$element/schools/school">
					...
					<xsl:choose>
						<xsl:when test="$schoolCount > 1">
							<a id="school_tooltip_for_{$id}" class="tooltip_trigger" href="javascript:void(0);"><xsl:value-of select="$schoolCount" /><xsl:text>&#160;</xsl:text><xsl:value-of select="$i18n.schools" /></a>
							<div class="tooltip hidden" id="school_tooltip_for_{$id}_container">
								<img class="alignmiddle" src="{$moduleImagePath}/school.png"/><xsl:text>&#160;</xsl:text>
				...
				<xsl:if test="$element/groups/group">
					...
					<xsl:choose>
						<xsl:when test="$groupCount > 1">
							<a id="group_tooltip_for_{$id}" class="tooltip_trigger" href="javascript:void(0);"><xsl:value-of select="$groupCount" /><xsl:text>&#160;</xsl:text><xsl:value-of select="$i18n.groups" /></a>
							<div class="tooltip hidden" id="group_tooltip_for_{$id}_container">
								<img class="alignmiddle" src="{$moduleImagePath}/group.png"/><xsl:text>&#160;</xsl:text>
