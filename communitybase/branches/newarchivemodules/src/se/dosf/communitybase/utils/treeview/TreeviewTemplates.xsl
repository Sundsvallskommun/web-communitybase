				...
				<ul rel="global">
					<li id="global_global" rel="global">
						...
						<xsl:attribute name="class">
							...
							<xsl:if test="not($globalAccess) or $globalAccess = 'false'">jstree-checkbox-disabled </xsl:if>
	...
	<xsl:template match="school" mode="tree">
		...
		<li id="schoolbase_{$schoolID}" rel="school">
			...
			<xsl:attribute name="class">
				...
				<xsl:if test="not($schoolAccess[schoolID = $schoolID]) and (not($globalAccess) or $globalAccess = 'false')">jstree-checkbox-disabled</xsl:if>
			</xsl:attribute>
