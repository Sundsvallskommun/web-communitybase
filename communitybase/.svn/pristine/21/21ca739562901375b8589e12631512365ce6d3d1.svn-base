...
import se.unlogic.webutils.populators.EnumPopulator;
...
public class GroupAdminModule extends AnnotatedAdminModule<CommunityGroup> implements CommunityModule {
	...
	private static final EnumPopulator<GroupAccessLevel> GROUP_ACCESS_LEVEL_POPULATOR = new EnumPopulator<GroupAccessLevel>(GroupAccessLevel.class, "GroupAccessLevel");
	...
	public ForegroundModuleResponse defaultMethod(HttpServletRequest req, HttpServletResponse res, CommunityUser user, URIParser uriParser, CommunityGroup group, ValidationException validationException) throws SQLException {
		...
		if(this.i18n && !this.systemInterface.getLanguageXSLs().isEmpty()) {
			...
			groupInfoElement.appendChild(languagesElement);
		}

		// Append started CommunityBase modules with ModuleType Public
		...
		if (firstpageModule != null) {
			...
			if (!moduleMap.isEmpty()) {
				...
				for (Entry<ForegroundModuleDescriptor, CommunityModule> moduleEntry : firstpageModule.getPublicModuleMap().entrySet()) {
					...
					XMLUtils.appendNewElement(doc, moduleElement, "link", moduleEntry.getValue().getFullAlias(group));
		...
		return new SimpleForegroundModuleResponse(doc, moduleDescriptor.getName(), new Breadcrumb(group.getName(), group.getDescription(), uriParser.getCurrentURI(true) + "/groupfirstpage/" + group.getGroupID(), URLType.FULL), getModuleBreadcrumb(group));
	}
		...
		if (req.getMethod().equalsIgnoreCase("POST")) {
			...
			try {
				...
				if (selectedUser != null) {
					...
					this.invitationDAO.addGroupInvitation(email, language, group, groupRelation, new Timestamp(System.currentTimeMillis() + ((long) this.invitationTimeout * MillisecondTimeUnits.DAY)), useGroupEmail);
				}
					...
					if(uriParser.get(3) != null && NumberUtils.isInt(uriParser.get(3)) && (selectedGroup = this.groupDAO.getGroup(Integer.valueOf(uriParser.get(3)), true)) != null) {
						...
						for(CommunityUser selectedUser : users) {
							...
							if(selectedGroup.getUsers() == null || (!selectedGroup.getUsers().contains(selectedUser))) {
								...
								if(!adminMoved && relation.getAccessLevel() == GroupAccessLevel.ADMIN) {
									adminMoved = true;
								}
	...
	private ForegroundModuleResponse updateModule(HttpServletRequest req, HttpServletResponse res, CommunityUser user, URIParser uriParser, CommunityGroup group, boolean enable) throws URINotFoundException, IOException, SQLException {
		...
		FirstpageModule currentFirstPageModule = this.getFirstPageModule();

		if (currentFirstPageModule == null) {
			this.redirectToDefaultMethod(req, res, group);
			return null;
		}

		for (ForegroundModuleDescriptor currentModuleDescriptor : currentFirstPageModule.getPublicModuleMap().keySet()) {

			if (currentModuleDescriptor.getModuleID() == moduleID) {
				...
				String className = currentModuleDescriptor.getClassname();
				
				for(FirstpageModule firstpageModule : firstpageModules) {

					for (ForegroundModuleDescriptor moduleDescriptor : firstpageModule.getPublicModuleMap().keySet()) {

						if (moduleDescriptor.getClassname().equals(className)) {
						
							boolean moduleEnabled = this.moduleDAO.isModuleEnabled(moduleDescriptor.getModuleID(), group);
		
							if (enable && !moduleEnabled) {
			
								log.info("User " + user + " enabling module " + moduleDescriptor + " in group " + group);
								
								this.moduleDAO.enableModule(moduleDescriptor, group);
