...
public class FilearchiveDAO extends BaseDAO {
	...
	public ArrayList<Section> getGroupSectionName(Integer groupID) throws SQLException {
		...
		ArrayListQuery<Section> query = new ArrayListQuery<Section>(this.dataSource, true, sql, SECTIONPOPULATOR);
	...
	public ArrayList<File> getGroupFiles(Integer groupID, Integer sectionID) throws SQLException{
		...
		ArrayListQuery<File> query = new ArrayListQuery<File>(this.dataSource, true, sql, POPULATOR);
	...
	public void addGroupSection(CommunityGroup group, Section section) throws SQLException{
		...
		UpdateQuery query = new UpdateQuery(this.dataSource, true, sql);
	...
	public Section getSection(Integer sectionID) throws SQLException {
		...
		ObjectQuery<Section> query = new ObjectQuery<Section>(this.dataSource, true, sql, SECTIONPOPULATOR);
	...
	public Section getSchoolSection(Integer sectionID) throws SQLException {
		String sql = "SELECT sectionID, name, schoolID AS id FROM filearchiveschoolsections WHERE sectionID = ?";
	...
	public File getGroupFile(int fileID, Integer groupID, boolean blob) throws SQLException {
		...
		ObjectQuery<File> query = new ObjectQuery<File>(this.dataSource,true,queryString,POPULATOR);
	...
	public File getGroupFile(int fileID, boolean blob) throws SQLException {
		...
		if(blob){
			...
			queryString =  "SELECT filearchivegroupfiles.*, filearchivegroupsections.name " +
					"FROM filearchivegroupfiles,filearchivegroupsections WHERE fileID = ? " +
					"AND filearchivegroupsections.sectionID = filearchivegroupfiles.sectionID";
	...
	public File getSchoolFile(int fileID, boolean blob) throws SQLException {
		...
		if(blob){
			...
			queryString =  "SELECT filearchiveschoolfiles.* , filearchiveschoolsections.name " +
					"FROM filearchiveschoolfiles,filearchiveschoolsections WHERE fileID = ? " +
					"AND filearchiveschoolsections.sectionID = filearchiveschoolfiles.sectionID";
	...
	public List<IdentifiedEvent> getEvents(CommunityGroup group, Timestamp lastLogin, boolean invertSchoolFileIDs) throws SQLException {
		...
		try{
			...
			String groupSQL = "SELECT \r\n" +
					"  filearchivegroupfiles.fileID as id,\r\n" +
			...
			ArrayListQuery<IdentifiedEvent> groupQuery = new ArrayListQuery<IdentifiedEvent>(connection, false, groupSQL, EVENT_POPULATOR);

			groupQuery.setInt(1, group.getGroupID());
			groupQuery.setTimestamp(2, lastLogin);

			List<IdentifiedEvent> groupEvents = groupQuery.executeQuery();

			String schoolSQL = "SELECT \r\n" +
					"  filearchiveschoolfiles.fileID as id,\r\n" +
			...
			ArrayListQuery<IdentifiedEvent> schoolQuery = new ArrayListQuery<IdentifiedEvent>(connection, false, schoolSQL, EVENT_POPULATOR);

			schoolQuery.setInt(1, group.getSchool().getSchoolID());
			schoolQuery.setTimestamp(2, lastLogin);

			List<IdentifiedEvent> schoolEvents = schoolQuery.executeQuery();

			if(schoolEvents == null && groupEvents == null){
				return null;
			}

			ArrayList<IdentifiedEvent> fileEvents = new ArrayList<IdentifiedEvent>();
