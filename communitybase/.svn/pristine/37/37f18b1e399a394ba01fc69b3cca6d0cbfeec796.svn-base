...
public class CommunityGroupDAO extends BaseDAO {
	...
	public ArrayList<CommunityGroup> getGroups(List<Integer> groupIDList, boolean users) throws SQLException {

		ArrayListQuery<CommunityGroup> query = new ArrayListQuery<CommunityGroup>(this.dataSource.getConnection(), true, "SELECT * FROM groups WHERE groupID IN (" + StringUtils.toCommaSeparatedString(groupIDList) + ") ORDER BY name", GROUP_POPULATOR);
	...
	public int getGroupCount() throws SQLException {
		...
		ObjectQuery<Integer> query = new ObjectQuery<Integer>(this.dataSource, true, "SELECT COUNT(groupID) FROM groups", IntegerPopulator.getPopulator());
	...
	public List<Character> getGroupFirstLetterIndex() throws SQLException {
		...
		return new ArrayListQuery<Character>(dataSource, true, "SELECT DISTINCT UPPER(LEFT(name, 1)) as letter FROM groups ORDER BY letter ", CharacterPopulator.getPopulator()).executeQuery();
	}
}
