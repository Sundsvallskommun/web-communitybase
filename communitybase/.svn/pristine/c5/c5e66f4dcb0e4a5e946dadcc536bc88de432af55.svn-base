			...
			if(this.sectionInterface.getSystemInterface().getSystemStatus() == SystemStatus.Started){
				...
				try {
					...
					if(users != null){
						...
						Map<Language, Map<ForegroundModuleDescriptor, CommunityModule>> moduleMap = new HashMap<Language, Map<ForegroundModuleDescriptor,CommunityModule>>();
						Entry<SectionDescriptor, Section> entry;
						boolean fallback = false;

						// i18n => get Firstpage module for each supported language (from each corresponding language section)
						if(this.i18n) {
							for(Language language : systemInterface.getLanguageXSLs().keySet()) {
								entry = systemInterface.getRootSection().getSectionCache().getEntry(language.getLanguageCode());
								if(entry == null) {
									log.warn("Language " + language + " has no corresponding section in the section hierarchy with the expected alias " + language.getLanguageCode() + ". Ignoring public modules for that language");
									continue;
								}
								if((firstpageModule = ModuleUtils.getCachedModule(entry.getValue(), FirstpageModule.class)) == null) {
									log.warn("No Firstpage module found in section with alias " + language.getLanguageCode() + ". Ignoring public modules for language " + language);
									continue;
								}
								moduleMap.put(language, firstpageModule.getPublicModuleMap());
							}
						}

						// No i18n => Try system default language  
						else if(systemInterface.getDefaultLanguage() != null) {
							if((entry = systemInterface.getRootSection().getSectionCache().getEntry(systemInterface.getDefaultLanguage().getLanguageCode())) != null) {
								if((firstpageModule = ModuleUtils.getCachedModule(entry.getValue(), FirstpageModule.class)) != null) {
									moduleMap.put(systemInterface.getDefaultLanguage(), firstpageModule.getPublicModuleMap());
								} else {
									log.warn("No Firstpage module found in section with alias " + systemInterface.getDefaultLanguage().getLanguageCode() + ". Fallback on this section");
									fallback = true;
								}
							} else {
								log.warn("System defailt language " + systemInterface.getDefaultLanguage() + " has no corresponding section in the section hierarchy with the expected alias " + systemInterface.getDefaultLanguage().getLanguageCode() + ". Fallback on this section");
								fallback = true;
							}
						}

						// No i18n and no system default language or using system default language failed => Try using this section (old behavior)
						if(!this.i18n && (systemInterface.getDefaultLanguage() == null || fallback)) {
							if((firstpageModule = ModuleUtils.getCachedModule(sectionInterface, FirstpageModule.class)) != null) {
								moduleMap.put(systemInterface.getDefaultLanguage(), firstpageModule.getPublicModuleMap()); // Yes, the map allows null keys
							} else {
								log.error("Unable to find Firstpage module in this section, aborting");
								return;
							}
						}

						if(moduleMap.isEmpty() || moduleMap.values().isEmpty()){
						...
						Transformer languageSpecificTransformer;
						Map<ForegroundModuleDescriptor, CommunityModule> map;

						for(CommunityUser user : users){

							useDefaultTransformer = true;
							language = null;
							languageSpecificTransformer = null;

							// If user has a language set and a corresponding transformer is cached => use that language specific transformer instead of default transformer
							try {
								if(this.i18n && (language = user.getLanguage()) != null) {
									if((xsltCacher = this.languageXSLs.get(language)) != null && (languageSpecificTransformer = xsltCacher.getTransformer()) != null) {
										...
										log.warn("User expects resume in an unsupported language " + language + ". Falling back on default transformer.");
									}
								}
							...
							// Use public modules of Firstpage module in in user language section (using user language as key) 
							// ..or fallback on public modules of Firstpage module in system default language section (using default language as key)
							// ..or fallback on public modules of Firstpage module in this section (using default language as key which could be null)
							if(language == null || (map = moduleMap.get(language)) == null) {
								map = moduleMap.get(systemInterface.getDefaultLanguage());
							}
							...
							for(CommunityGroup group : user.getGroups()){
								...
								for(Integer moduleID : groupModuleIDs){
									...
									for (Entry<ForegroundModuleDescriptor, CommunityModule> moduleEntry : map.entrySet()) {
