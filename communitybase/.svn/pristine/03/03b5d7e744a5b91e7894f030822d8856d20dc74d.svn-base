...
(function($) {
	...
	var defaults = {
			...
			labelHeight: 25,
			firstDayOfWeek: 0,
			dateLabelTitle: 'Add calendarpost',
			dateLabelLinkUrl: '#',
			...
			onAddMoreEventInfo: function(event) { return true; } 
		};
	...
	jQuery.J.DrawCalendar = function(dateIn){
		...
		do {
			...
			for (var i=0; i<7; i++) {
				...
				//DateBox Events
				var dateLink = jQuery('<div class="DateLabel"><a href="' + defaults.dateLabelLinkUrl + '" title="' + defaults.dateLabelTitle + '">' + _currentDate.getDate() + '</a></div>').click(function(e) {
			...
			jQuery.each(calendarEvents, function(){
				...
				if(sDt) {
					if ((sDt >= _beginDate) && (sDt <= _endDate)) {			
						...
						defaults.onAddMoreEventInfo(ev, event);
