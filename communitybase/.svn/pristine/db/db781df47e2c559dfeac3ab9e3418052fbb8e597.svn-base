...
public class CommunitySchoolDAO extends BaseDAO {
	...
	public List<School> getSchools(boolean getAdmins, boolean getGroups) throws SQLException {
		...
		ArrayListQuery<School> query = new ArrayListQuery<School>(dataSource, "SELECT * FROM schools ORDER BY name", POPULATOR);
	...
	public List<School> getSchools(CommunityUser user, boolean getAdmins, boolean getGroups) throws SQLException {
		...
		try {
			...
			ArrayListQuery<School> query = new ArrayListQuery<School>(connection, false, "SELECT schools.* FROM schooladmins INNER JOIN schools ON (schooladmins.schoolID=schools.schoolID) WHERE (schooladmins.userID = ?) ORDER BY schools.name", POPULATOR);
			...
			if (schools != null && (getAdmins || getGroups)) {
				...
				for (School school : schools) {
					...
					if (getAdmins) {
						school.setAdmins(this.userDAO.getUsers(school, connection));
					}

					if (getGroups) {
						school.setGroups(this.groupDAO.getGroups(school, connection));
					}
				}
			}
	...
	public List<School> getSchools(CommunityUser user, boolean getAdmins, boolean getGroups, Connection connection) throws SQLException {
	...
	public ArrayList<School> getSchools(List<Integer> schoolIDList) throws SQLException {

		ArrayListQuery<School> query = new ArrayListQuery<School>(this.dataSource.getConnection(), true, "SELECT * FROM schools WHERE schoolID IN (" + StringUtils.toCommaSeparatedString(schoolIDList) + ")", POPULATOR);
	...
	public void add(School school) throws SQLException {
		...
		UpdateQuery query = new UpdateQuery(dataSource, "INSERT INTO schools VALUES (null,?)");
	...
	public void update(School school) throws SQLException {
		...
		UpdateQuery query = new UpdateQuery(dataSource, "UPDATE schools SET name = ? WHERE schoolID = ?");
	...
	public void delete(School school) throws SQLException {
		...
		UpdateQuery query = new UpdateQuery(dataSource, "DELETE FROM schools WHERE schoolID = ?");
