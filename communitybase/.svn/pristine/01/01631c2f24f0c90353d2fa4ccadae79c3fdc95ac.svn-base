...
public class MembersDAO extends BaseDAO {
	...
	public boolean userIsHidden(CommunityGroup group, CommunityUser user) throws SQLException {
		...
		BooleanQuery query = new BooleanQuery(this.dataSource, true, "SELECT userID FROM " + HIDDEN_USERS_TABLE + " WHERE groupID = ? AND userID = ?");
	...
	public List<Integer> getHiddenUserIDs(CommunityGroup group) throws SQLException {
		...
		ArrayListQuery<Integer> query = new ArrayListQuery<Integer>(this.dataSource, true, "SELECT userID FROM " + HIDDEN_USERS_TABLE + " WHERE groupID = ?", IntegerPopulator.getPopulator());
	...
	public void setVisible(CommunityGroup group, CommunityUser user) throws SQLException {
		...
		UpdateQuery query = new UpdateQuery(this.dataSource, true, "DELETE FROM " + HIDDEN_USERS_TABLE + " WHERE userID = ? AND groupID = ?");
	...
	public void setInVisible(CommunityGroup group, CommunityUser user) throws SQLException {
		...
		UpdateQuery query = new UpdateQuery(this.dataSource, true, "INSERT INTO " + HIDDEN_USERS_TABLE + " VALUES(?,?)");
