...
public class EmailResumeSender extends AnnotatedForegroundModule implements Runnable {
					...
					if (users != null) {
						...
						for (CommunityUser user : users) {
							...
							if (user.getLastLogin() == null) {
								...
								// User has never received an e-mail resume use
								// last login timestamp
								lastResume = user.getLastLogin();

							} else if (user.getLastLogin().getTime() > user.getLastResume().getTime()) {

								// User has logged in since the last resume was
								// sent, use last login timestamp
								lastResume = user.getLastLogin();
							} else {
							...
							// Global resume
							List<String> globalResumeHTMLs = new ArrayList<String>();

							try {

								if (!CollectionUtils.isEmpty(user.getCommunityGroups())) {

									List<Integer> distinctModuleIDs = this.moduleDAO.getDistinctEnabledModules(new ArrayList<CommunityGroup>(user.getCommunityGroups()));

									if (distinctModuleIDs == null) {
										continue;
									}

									for (Integer moduleID : distinctModuleIDs) {

										Entry<ForegroundModuleDescriptor, CommunityModule> moduleEntry = this.getModuleEntry(moduleID, moduleMap);

										if (moduleEntry != null && moduleEntry.getValue() instanceof EmailResumeProvider) {

											String globalResumeHTML = ((EmailResumeProvider) moduleEntry.getValue()).getGlobalEmailResumeHTML(user);

											if (!StringUtils.isEmpty(globalResumeHTML)) {

												globalResumeHTMLs.add(globalResumeHTML);
								...
								log.error("Unable to get global resume for user " + user, e);

							}

							// Ordinary group resume
							...
							for (CommunityGroup group : user.getCommunityGroups()) {
								...
								for (Integer moduleID : groupModuleIDs) {
									...
									Entry<ForegroundModuleDescriptor, CommunityModule> currentModuleEntry = this.getModuleEntry(moduleID, moduleMap);

									if (currentModuleEntry != null) {

										try {

											CommunityModule module = currentModuleEntry.getValue();

											List<? extends Event> moduleEvents = module.getGroupResume(group, user, lastResume);
							...
							if (globalResumeHTMLs.isEmpty() && userEventMap.isEmpty()) {
								continue;
							}
							...
							if (!globalResumeHTMLs.isEmpty()) {

								Element globalResumeElement = doc.createElement("GlobalResume");
								documentElement.appendChild(globalResumeElement);

								for (String html : globalResumeHTMLs) {

									XMLUtils.appendNewElement(doc, globalResumeElement, "globalResumeHTML", html);
	...
	protected Entry<ForegroundModuleDescriptor, CommunityModule> getModuleEntry(Integer moduleID, Map<ForegroundModuleDescriptor, CommunityModule> moduleMap) {
