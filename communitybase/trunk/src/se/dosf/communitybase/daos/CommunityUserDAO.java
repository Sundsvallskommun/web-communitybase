...
public class CommunityUserDAO extends BaseDAO {
	...
	public List<CommunityUser> getUsers(List<Integer> userIDList, boolean groups) throws SQLException {

		ArrayList<CommunityUser> users = new ArrayListQuery<CommunityUser>(this.dataSource, true, "SELECT * FROM users WHERE userID IN (" + StringUtils.toCommaSeparatedString(userIDList) + ") ORDER BY firstname, lastname", POPULATOR).executeQuery();
	...
	public List<CommunityUser> getUsers(Integer groupID) throws SQLException {
		...
		ArrayListQuery<CommunityUser> query = new ArrayListQuery<CommunityUser>(this.dataSource, true, "SELECT users.userID, users.`password`, users.firstname, users.lastname, users.email, users.admin, users.enabled, users.added, users.lastlogin, users.phoneHome, users.phoneWork, " + "users.resume, users.lastresume, users.phoneMobile " + "FROM groupusers INNER JOIN users ON (groupusers.userID=users.userID) " + "WHERE (groupusers.groupID = ?) ORDER BY users.firstname, users.lastname;", POPULATOR);
	...
	public List<CommunityUser> getUsers(boolean getSchools, boolean getGroups, boolean getSchoolAdmins, List<GroupAccessLevel> accessLevels) throws SQLException {
		...
		List<CommunityUser> users = new ArrayListQuery<CommunityUser>(dataSource, true, sql, POPULATOR).executeQuery();
	...
	public int getUserCount() throws SQLException {
		...
		ObjectQuery<Integer> query = new ObjectQuery<Integer>(this.dataSource, true, "SELECT COUNT(userID) FROM users", IntegerPopulator.getPopulator());
	...
	public List<Character> getUserFirstLetterIndex(UserField filteringField) throws SQLException {
		...
		return new ArrayListQuery<Character>(this.dataSource, true, "SELECT DISTINCT UPPER(LEFT(" + filteringField.toString().toLowerCase() + ", 1)) as letter FROM users ORDER BY letter ", CharacterPopulator.getPopulator()).executeQuery();
	}
	...
	public List<CommunityUser> getUsers(UserField filteringField, Order order, char startsWith, boolean groups) throws SQLException {
		...
		ArrayListQuery<CommunityUser> query = new ArrayListQuery<CommunityUser>(this.dataSource, true, sql, POPULATOR);
