...
public class LinkDAO extends BaseDAO {
	...
	public Link get(Integer linkID, CommunityUser user) throws SQLException {
	...
	public boolean checkAccess(CommunityUser user, Integer linkID) throws SQLException{
		...
		try{
			...
			BooleanQuery query = new BooleanQuery(connection, false, "SELECT * FROM globallinks WHERE linkID = ?");
			...
			if(!CollectionUtils.isEmpty(user.getGroups())){
				...
				query = new BooleanQuery(connection, false, "SELECT * FROM linkgroups WHERE linkID = ? AND groupID IN (?" + StringUtils.repeatString(",?", user.getGroups().size() - 1) + ")");
				...
				for(Group group : user.getGroups()){
					...
					query.setInt(i++, group.getGroupID());
				}
				...
				query = new BooleanQuery(connection, false, "SELECT * FROM schoollinks WHERE linkID = ? AND schoolID IN (SELECT DISTINCT schoolID FROM groups WHERE groupID IN (?" + StringUtils.repeatString(",?", user.getGroups().size() - 1) + "))");
			...
			if(!CollectionUtils.isEmpty(user.getSchools())){
				...
				query = new BooleanQuery(connection, false, "SELECT * FROM schoollinks WHERE linkID = ? AND schoolID IN (?" + StringUtils.repeatString(",?", user.getSchools().size() - 1) + ")");
				...
				for(School school : user.getSchools()){
					...
					query.setInt(i++, school.getSchoolID());
				}
				...
				query = new BooleanQuery(connection, false, "SELECT * FROM linkgroups WHERE linkID = ? AND groupID IN (SELECT groupID FROM groups WHERE schoolID in (?" + StringUtils.repeatString(",?", user.getSchools().size() - 1) + "))");
