		...
		<div class="contentitem">
			...
			<form method="post" name="mysettings" id="mysettings" action="{/document/requestinfo/uri}">
				...
				<div class="content-box">
					...
					<h1 class="header">
						<span><xsl:value-of select="$RequestNewPassword.header" />?</span>
					</h1>
					...
					<div class="content">
						...
						<div class="floatleft full bigmarginbottom">
							<div class="floatleft ten"><xsl:value-of select="$RequestNewPassword.email" />:</div>
							<div class="floatleft">
		...
		<html>
			...
			<body>
				<table>
					...
					<tr>
						<td>
							<xsl:value-of select="$RequestNewPasswordEmail.about.part1" /><xsl:text>&#x20;</xsl:text><a href="{/document/systemURL}"><xsl:value-of select="$RequestNewPasswordEmail.about.part2" /></a>.
						</td>
					</tr>
					<tr>
						<td>
							<xsl:value-of select="$RequestNewPasswordEmail.municipalitylink" /><xsl:text>&#x20;</xsl:text><xsl:value-of select="/document/municipalityName" /><xsl:text>&#x20;</xsl:text>
