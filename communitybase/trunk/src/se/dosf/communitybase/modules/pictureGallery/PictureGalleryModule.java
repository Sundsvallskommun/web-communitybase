...
/**
 * TODO: Implement "access control" where it is missing
 * 
 * At some places, i.e. dowloadGallery, showGallery, the only access control is done at module level (validating user group membership in superclass)
 * 
 * After that, the user can request any gallery for download/show, including galleries in other groups
 *
 */
public class PictureGalleryModule extends AnnotatedCommunityModule implements CommunityModule {
	...
	static {
		...
		SETTINGDESCRIPTORS.add(SettingDescriptor.createCheckboxSetting("allowsGalleryDownload", "Allows gallery download", "Control wheter or not gallery download is allowed", false));
	...
	public ForegroundModuleResponse defaultMethod(HttpServletRequest req, HttpServletResponse res, CommunityUser user, URIParser uriParser, CommunityGroup group) throws Exception {
		...
		if(allowsGalleryDownload) {
			XMLUtils.appendNewElement(doc, picturegallery, "allowsGalleryDownload", allowsGalleryDownload);
		}
	...
	private ForegroundModuleResponse showGallery(HttpServletRequest req, HttpServletResponse res, CommunityUser user, URIParser uriParser, CommunityGroup group, RelationType type) throws URINotFoundException, NumberFormatException, SQLException {
		...
		if (uriParser.size() < 4 || uriParser.size() > 5 || !NumberUtils.isInt(uriParser.get(3)) || (gallery = this.galleryDAO.getGallery(Integer.valueOf(uriParser.get(3)), true, type)) == null) {
			...
			if(allowsGalleryDownload) {
				XMLUtils.appendNewElement(doc, showGallery, "allowsGalleryDownload", allowsGalleryDownload);
			}
	...
	@GroupMethod
	public ForegroundModuleResponse downloadGroupGallery(HttpServletRequest req, HttpServletResponse res, CommunityUser user, URIParser uriParser, CommunityGroup group) throws Exception {

		return this.downloadGallery(req, res, user, uriParser, group, RelationType.GROUP);
	}

	@GroupMethod
	public ForegroundModuleResponse downloadSchoolGallery(HttpServletRequest req, HttpServletResponse res, CommunityUser user, URIParser uriParser, CommunityGroup group) throws Exception {

		return this.downloadGallery(req, res, user, uriParser, group, RelationType.SCHOOL);
	}

	private ForegroundModuleResponse downloadGallery(HttpServletRequest req, HttpServletResponse res, CommunityUser user, URIParser uriParser, CommunityGroup group, RelationType type) throws Exception {

		if(this.allowsGalleryDownload) {

			Gallery gallery = null;

			if (uriParser.size() != 4 || (gallery = this.galleryDAO.getGallery(Integer.valueOf(uriParser.get(3)), false, type)) == null) {
			...
			List<Picture> pictures = this.pictureDao.findByGalleryID(gallery.getSectionID(), false, false, true, type);
			...
			if(!CollectionUtils.isEmpty(pictures)) {
				...
				try {
					...
					res.setHeader("Content-Disposition", "inline; filename=\"" + FileUtils.toValidHttpFilename(gallery.getSectionName() + ".zip"));
					...
					for(Picture picture : pictures) {
						...
						ZipEntry zipEntry = new ZipEntry(picture.getFileID() + "_" + picture.getFilename());

						InputStream inputStream = null;

						try {

							Blob blob = picture.getLargePic();
	...
	public Document createDocument(HttpServletRequest req, URIParser uriParser, CommunityGroup group, CommunityUser user) {
		...
		if(this.diskThreshold != null) {
			XMLUtils.appendNewElement(doc, document, "diskThreshold", this.diskThreshold * BinarySizes.MegaByte);
		}

		if(AccessUtils.checkAccess(user, group.getSchool())) {
			XMLUtils.appendNewElement(doc, document, "isSchoolAdmin", "true");
		} else if(AccessUtils.checkAccess(user, group, GroupAccessLevel.ADMIN, GroupAccessLevel.PUBLISHER)) {
			XMLUtils.appendNewElement(doc, document, "isGroupAdmin", "true");
		}
